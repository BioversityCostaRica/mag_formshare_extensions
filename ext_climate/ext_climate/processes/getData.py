...
def FormsWithRepo(self):
    result = (
        self.request.dbsession.query(Odkform.form_name, Odkform.form_schema)
        .filter(Odkform.project_id == self.activeProject["project_id"])
...
def formSummary(self, fname):
    ...
    records = self.request.dbsession.execute(
        "select count(*) as total from %s.maintable" % fname
    ).fetchone()
    ...
    for r in records:
        if "prestada" in r.des:
            data["own"].append(
                ["Prestadas", "%.1f" % (100 / int(data["count"]) * r.ln)]
            )
...
def getSensiMaps(self, div, prj):
    if div == "c":
        i_path = (
            self.request.registry.settings.get("repository.path", "")
            + "maps/%s/sensitivity-canton.shp" % prj
        )
    elif div == "d":
        i_path = (
            self.request.registry.settings.get("repository.path", "")
            + "maps/%s/sensitivity-distrito.shp" % prj
        )
